...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, act='relu'):
        ...
        # self.weight.data = self.weight.sigmoid()
        # self.bias.data = 1-self.weight
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            ...
            # b = self.bias
        # return x
        return (x * 0.8 + x.mean(dim=1, keepdim=True) * 0.2)
        # return (x + x.mean(dim=1, keepdim=True) )* 0.5
        # return (x * 0.2 + x.mean(dim=1, keepdim=True) * 0.8)
        # return  x.mean(dim=1, keepdim=True) 
        # return a * x + b * x.mean(dim=1, keepdim=True)
