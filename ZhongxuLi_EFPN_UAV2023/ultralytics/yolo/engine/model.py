...
class YOLO:
    ...
    def _new(self, cfg: str, verbose=True):     # 通过配置文件读取模型
        """
...
#         print(f'Transferred {len(csd)}/{len(self.model.state_dict())} items')
    ...
    def _load(self, weights: str):       # 通过权重导入模型
        """
    ...
    def train(self, **kwargs):
        ...
        if kwargs.get("cfg"):
            ...
            overrides = yaml_load(check_yaml(kwargs["cfg"]), append_filename=False)    # 修改
        ...
        self.trainer.model = self.model      # 修改  断点续训注释掉 正常训练开启
