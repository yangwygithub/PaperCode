...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment, SimSPPF, SimConv, SimCSPSPPF, CBAM, CAM, BiFPN_Add2, BiFPN_Add3)
...
from ultralytics.nn.cspstage import CSPStage
from ultralytics.nn.repblock import RepBlock
from ultralytics.nn.evc_blocks import EVCBlock
from ultralytics.nn.mobileone import MobileOne
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, SimSPPF, SimCSPSPPF, SimConv, DWConv, Focus,
                BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CSPStage, RepBlock, EVCBlock, MobileOne, CBAM}:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, CSPStage, RepBlock, EVCBlock, MobileOne, CBAM}:
