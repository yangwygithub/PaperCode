...
__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'MSSPPF', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
...
class MSSPPF(nn.Module):
    def __init__(self, c1, c2, k=(5, 9, 13)):
        super().__init__()
        c_ = c1 // 2
        self.cv1 = SimConv(c1, c_, 1, 1)
        self.cv2 = SimConv(c_ * 10, c2, 1, 1)
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat(pool_feats, 1))
...
class hC2f(nn.Module):            # c2f_horblock
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(HorBlock(self.c) for _ in range(n))
